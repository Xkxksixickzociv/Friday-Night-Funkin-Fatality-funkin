function onCreate() --FUCKING SHIT FUCK FUCK FUCKITY FUCKING SPAGHETTI CODE FUCKING SHIT BALLS SHIT CRAP PISS FUCKING SHIT
	makeLuaSprite('AwardBG', 'empty', 10, 450)
	makeGraphic('AwardBG', 337.5, 112.5, '000000')
	setObjectCamera('AwardBG', 'other')
	
	makeAnimatedLuaSprite('AwardIcon', 'Awards/Scorpion2-Award', 10, 450)
	addAnimationByPrefix('AwardIcon', 'AwardFC', 'AwardFC', 24, false)
	scaleObject('AwardIcon', 0.75, 0.75)
	setObjectCamera('AwardIcon', 'other')
	
	makeLuaText('AwardTitle', 'Sample Text', 200, 125, 460)
	makeLuaText('AwardSub', 'Sample Text', 200, 125, 500)
...
function onEvent(name, value1, value2)
	...
	if name == 'AwardCheck2' then
		if value1 == 'fc' then
			if getProperty('songMisses') < 9999 then
				--text shit
				setTextString('AwardTitle', 'TOASTED´NT')
				setTextString('AwardSub', 'win against Quan chi´s Subordinates')
